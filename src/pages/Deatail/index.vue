...
export default {
  ...
  methods: {
    //todo 发送请求的方法
    getDetailList() {
      this.$store.dispatch("getDetail", this.skuId);
    },
    //todo 定义变绿喷雾
    ...
    async addShopCart() {
      ...
      try {
        const result = await this.$store.dispatch("addOrUpdateCart", {skuId,
          skuNum
          
        })
        alert("强袭自由出击")
        sessionStorage.setItem("SKUINFO_KEY", JSON.stringify(this.skuInfo))
        this.$router.push("/addOrUpdataCarts")
      } catch (error) {
        alert(error.message);
      }
    },
